...
Proportion of baseline facilities that were not matched by LGA. (Total: 26533, ~30% of 93085)

```{r echo=F, warning=F, message=F, comment=NA, fig.height=6, fig.width=10, cache=TRUE}
  library(classInt)
  #brks <- classIntervals((matched$total_baseline - matched$left_baseline) / matched$total_baseline, n=5, style="fisher")$brks
  matched$UnmatchedRatio <- cut((matched$left_baseline) / matched$total_baseline, breaks=c(0,.3,.6,.9,1), include.lowest=T, 
                              labels=c("0-30%", "30-60%", "60-90%", "90% +"))
```
```{r echo=F, warning=F, message=F, comment=NA, fig.height=7, fig.width=13, cache=TRUE}
#source("../NigeriaPresentationUtils.R")
  lga_map(geom_map(data = matched, aes(map_id = lga_id, fill = UnmatchedRatio), map=lgas))
```
...
Total missing facilities: 67463, ~50% of facility list total (134015).
