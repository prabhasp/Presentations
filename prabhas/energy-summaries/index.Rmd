...
bin_and_map <- function(df, colname, breaks=c(0,.3,.6,.9,1), labels=c("0-30%", "30-60%", "60-90%", "90% +"), nostatename=FALSE) {
  binname <- paste(colname,'bin',sep='.')
  df[,binname] <- cut(df[,colname], breaks=breaks, include.lowest=T, labels=labels)
  lga_map(geom_map(data=df, aes_string(map_id='lga_id', fill=binname), map=lgas), nostatenames=nostatename)
}
boxplot_by_zone <- function(df, colname, zonecolname='zone') {
  ggplot(data=df, aes_string(x=zonecolname, y=colname, fill=zonecolname)) + geom_boxplot()  
}
```
## Outline
...
facagg <- ddply(idata.frame(fac), .(lga_id), function(df) {
  data.frame(
    ...
    HealthFunctionalPower = bool_proportion(df$functional_power, df$sector=="HEALTH"),
    EducationFunctionalPower = bool_proportion(df$functional_power, df$sector=="EDUCATION"),
    FunctionalPower = bool_proportion(df$functional_power),
    HasGrid = bool_proportion(df$grid),
    HasSolar = bool_proportion(df$solar),
    HasGenerator = bool_proportion(df$genset),
...
```{r echo=F, warning=F, message=F, comment=NA, fig.height=4, fig.width=4}
boxplot_by_zone(facagg, 'HasGrid')
boxplot_by_zone(facagg, 'HasGenerator')
boxplot_by_zone(facagg, 'HasSolar')
bin_and_map(facagg, 'HasGrid', nostatename=TRUE)
bin_and_map(facagg, 'HasGenerator', nostatename=TRUE)
bin_and_map(facagg, 'HasSolar', nostatename=TRUE)
```
