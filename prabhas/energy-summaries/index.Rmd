...
boxplot_by_zone <- function(df, colname, zonecolname='zone', title=NA) {
    ...
    geom_violin(alpha=0.3, width=0.5, aes_string(color=zonecolname)) +
...
ggplot(data=facagg, aes(x=zone, fill=DominantElectricSystem)) + geom_bar() +
  scale_fill_brewer(type="qual", palette=2) 
lga_map(geom_map(data = facagg, aes(map_id = lga_id, fill = DominantElectricSystem), map=lgas), filltype="qual") + labs(tile="DominantElectricSystem")
```
---
